...
public class MainActivity extends Activity implements SensorEventListener {
    RadarView radar; TextView status; Spinner category, radius; LocationManager lm; Location here; SensorManager sm; Sensor rot; float heading=0;
    final String[] cats={"Potenziali clienti","Ristoranti","Bar","Hotel","Supermercati","Distributori","Farmacie","Officine","Aziende"};
    final String[] tags={"commercial","amenity~\"restaurant|fast_food\"","amenity=\"cafe\"","tourism=\"hotel\"","shop=\"supermarket\"","amenity=\"fuel\"","amenity=\"pharmacy\"","shop=\"car_repair\"","office"};
    @Override public void onCreate(Bundle b){super.onCreate(b); build(); lm=(LocationManager)getSystemService(LOCATION_SERVICE); if(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED) requestPermissions(new String[]{Manifest.permission.ACCESS_FINE_LOCATION,Manifest.permission.ACCESS_COARSE_LOCATION},7); else locate(); sm=(SensorManager)getSystemService(SENSOR_SERVICE); rot=sm.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR); if(rot!=null)sm.registerListener(this,rot,SensorManager.SENSOR_DELAY_UI);}
    void build(){
    ...
    void scan(){ if(here==null){status.setText("Sto cercando la posizione GPS…");return;} status.setText("Ricerca in corso…"); new Thread(()->{
        try{
            String t=tags[category.getSelectedItemPosition()];
            String filter=t.equals("office")?"[\\\"office\\\"]":t.equals("commercial")?"[\\\"name\\\"][\\\"shop\\\"~\\\".\\\"]":"["+t+"]";
            ...
            HttpURLConnection c=(HttpURLConnection)u.openConnection(); c.setRequestProperty("User-Agent","RadarAttivita/1.1"); c.setConnectTimeout(15000); c.setReadTimeout(25000);
    ...
    @Override public void onSensorChanged(SensorEvent e){float[]rm=new float[9],o=new float[3];SensorManager.getRotationMatrixFromVector(rm,e.values);SensorManager.getOrientation(rm,o);heading=(float)Math.toDegrees(o[0]);radar.invalidate();}
